<template>
  <div class="home">
        ...
        <v-progress-linear
          ...
          <v-row>
            ...
            <v-col md="6" class="mt-5">
              ...
              <v-card-actions>
                <v-row class="d-flex justify-space-around py-2">
                  <v-btn color="green" @click="thumbsUp">
                    <v-icon>mdi-thumb-up</v-icon>
                  </v-btn>
                  <v-btn
                    color="red"
                    @click="thumbsDown"><v-icon>mdi-thumb-down</v-icon></v-btn>
                </v-row>
              </v-card-actions>
            </v-col>
          </v-row>
    ...
    <v-row>
      <v-col class="text-center">Powered by <a href="https://www.themoviedb.org/">The Movie Database</a> </v-col>
    </v-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchMovies(page){
      const res = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=bad010568c2b6bd837d2c6c657837ac3&language=en-US&sort_by=popularity.desc&include_adult=true&include_video=false&page=${page}`);
    ...
    async thumbsUp() {
      ...
      if(this.partnerId) {
        ...
        const partnerLikedSnapshot = await partnerRef.collection("likedMovies").where("id", "==", this.currentMovie.id).get();
  ...
  computed: {
    ...
    userMovieApiPage() {
      return this.$store.state.user.userMovieApiPage;
    },
    authUserId() {
